    ...
    if (imagePath && await fs.pathExists(imagePath)) {
      ...
      try {
        ...
        try {
          ...
          console.log('PNG upload failed, posting without image');
        }
      }
    }
...
app.get('/api', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      post: '/api/post (POST)',
      startAuto: '/api/start-auto (POST)',
      stopAuto: '/api/stop-auto (POST)',
      updateInterval: '/api/update-interval (POST)',
      reset: '/api/reset (POST)',
      ping: '/ping',
      testZernio: '/api/test-zernio'
    }
  });
});
